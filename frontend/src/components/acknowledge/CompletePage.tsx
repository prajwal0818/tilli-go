...
export default function CompletePage() {
  ...
  useEffect(() => {
    if (!taskId || !token) {
      ...
      setError('Invalid completion link — missing task_id or token.');
      return;
    }
    ...
    api
      .get<CompleteResponse>('/complete', { params: { task_id: taskId, token } })
      .then((res) => {
        if (!cancelled) {
      ...
      >
        ...
        {status === STATUS.LOADING && (
          <div className="flex flex-col items-center gap-4">
            ...
            />
            <p className="text-sm text-text-secondary">Completing task...</p>
          </div>
        )}
          ...
          >
            ...
            <table className="w-full text-left mt-2">
              <tbody>
                ...
                {data.actualEndTime && (
                  <tr>
                    <td className="px-3 py-2 text-sm text-text-secondary font-medium border-b border-border">Completed At</td>
                    <td className="px-3 py-2 text-sm text-text-primary border-b border-border">
                      {new Date(data.actualEndTime).toLocaleString()}
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        {status === STATUS.ERROR && (
          ...
          >
            ...
            <h2 className="text-lg font-semibold text-red-800">Completion Failed</h2>
